...
export function SankeyChart({ data, svgRef }: { data: SegmentSankeyData; svgRef?: RefObject<SVGSVGElement | null> }) {
  ...
  // ── Render ─────────────────────────────────────────────────────────────────
  return (
    <div className="sankey-wrap" style={{ width: "100%" }}>
      <div
        className="sankey-head"
        style={{
          display: "flex",
          flexWrap: "wrap",
          alignItems: "center",
      ...
      >
        ...
        {data.geographyOnly && data.segments.length > 0 && (
          <span className="sankey-meta">· Revenue by region</span>
        )}
        {data.segmentPeriod && data.segments.length > 0 && data.segmentPeriod !== data.period && (
          <span className="sankey-meta">· Segments: {data.segmentPeriod}</span>
        )}
        {/^FY\d{4}$/i.test(data.period ?? "") && (
          <span
            className="sankey-meta"
        ...
        <span className="sankey-meta" style={{ marginLeft: "auto" }}>in {data.currency}</span>
      </div>

      <svg
        ref={svgRef}
        viewBox={`0 0 ${VW} ${VH}`}
        className="w-full sm:max-h-[800px]"
        style={{
          display: "block",
