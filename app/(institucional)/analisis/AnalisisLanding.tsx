...
import { AnalisisPopulares } from "@/components/institucional/AnalisisPopulares";
import { Verdict, BarChart, LineChart, Flow, Poll, FileDown } from "@/components/institucional/icons";

const FEATURES: { icon: ReactNode; title: string; body: string }[] = [
  { icon: <Verdict />, title: "Veredicto", body: "Rating BUY · HOLD · AVOID con convicción declarada y rationale escrito." },
...
export function AnalisisLanding() {
  const router = useRouter();

  // Entrar al reporte = navegar a /analisis?ticker=X (misma ruta + query).
  // AnalisisSwitch detecta el ticker y monta el reporte en lugar de esta landing.
  function handleSearch(ticker: string) {
    router.push(`/analisis?ticker=${encodeURIComponent(ticker.trim().toUpperCase())}`);
  }
  ...
  return (
    <main className="site">
      ...
      {/* Las más analizadas — leaderboard de tickers populares (7 días), justo tras
          el hero: máximo momento de intención + prueba social. Entra a un reporte
          con el mismo handleSearch que el buscador. */}
      <AnalisisPopulares onSelect={handleSearch} />
