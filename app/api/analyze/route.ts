...
// wait below guarantees a stuck entry can never hang a request.
...
export async function POST(req: NextRequest) {
  ...
  // client that opens /api/analyze and walks away leaves the `for await` loop
  // draining the completion to the end — billing the full output. req.signal
  // fires on disconnect; the stream's cancel() (client closed the SSE) aborts
  // too. Either way we stop paying for tokens nobody will read.
